...
// DNA names that are hardcoded should start with '-' to prevent collisions with input code

/* naming conventions for hardcoded DNA name prefixes
...
public:
	...
	string getCpp(const string& pn); // will throw an error if the DNA name doesn't exist
...
public:
	...
	void name(const string& in); // converts a DNA name to a posibly different C++ name
