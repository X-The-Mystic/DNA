...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		{
			throw DNAError("AndAction created with first action that does not return Bool", INTERNAL_ERROR);
		}
		
		if (secondAction->getReturnType()!=Bool)
		{
			throw DNAError("AndAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
...
public:
		...
		:ActionData(Bool, Void, Void)
	{
		...
		if (firstAction->getReturnType()!=Bool)
		{
			throw DNAError("OrAction created with first action that does not return Bool", INTERNAL_ERROR);
		}
		
		if (secondAction->getReturnType()!=Bool)
		{
			throw DNAError("OrAction created with second action that does not return Bool", INTERNAL_ERROR);
		}
	}
