...
void AstList::resolveAction()
{
	...
	{
		throw DNAError("AstList given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstExpression::resolveAction()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw DNAError("AstExpression given non void input", INTERNAL_ERROR, getToken());
	}
	
	if (rightIn->isType())
	{
		throw DNAError("types must be declared as constants", SOURCE_ERROR, rightIn->getToken());
	}
	else if (center->isType() || center->isFunctionWithOutput() || leftIn->isType())
	{
		throw DNAError("a function implementation got into an expression node somehow", INTERNAL_ERROR, center->getToken());
	}
	else
	{
...
void AstConstExpression::resolveConstant()
{
	if (!inLeftType->isVoid() || !inRightType->isVoid())
	{
		throw DNAError("AstConstExpression given non void input", INTERNAL_ERROR, getToken());
	}
...
void AstOpWithInput::resolveAction()
{
	if (token->getOp()==ops->ifOp)
	{
		...
		if (leftIn.empty())
		{
			throw DNAError("'?' must have a conditional to its left", SOURCE_ERROR, token);
		}
		else if (leftIn.size()!=1)
		{
			throw DNAError("'?' can only have one conditional to its left", SOURCE_ERROR, token);
		}
		...
		if (rightIn.empty())
		{
			throw DNAError("'?' must have a statement to its right", SOURCE_ERROR, token);
		}
		...
		{
			throw DNAError("'?' can only have 1 or 2 '|' seporated expressions to its right", SOURCE_ERROR, token);
		}
	}
	else if (token->getOp()==ops->loop)
	{
		...
		if (rightIn.size()>1)
		{
			throw DNAError("'@' followed by multiple expressions", SOURCE_ERROR, token);
		}
		
		if (leftIn.size()==0)
		{
			throw DNAError("condition needed before '@'", SOURCE_ERROR, token);
		}
		else if (leftIn.size()==1)
		{
		...
		{
			throw DNAError("chain of length "+to_string(leftIn.size())+"preceding '@', it should be length 1-3", SOURCE_ERROR, token);
		}
	...
	else if (token->getOp()==ops->andOp || token->getOp()==ops->orOp)
	{
		...
		{
			throw DNAError("'"+token->getOp()->getText()+"' can not be sent '|' separated sequence", SOURCE_ERROR, getToken());
		}
		
		if (leftIn.size()!=1 || rightIn.size()!=1)
		{
			throw DNAError("'"+token->getOp()->getText()+"' must be given a left and right input", SOURCE_ERROR, getToken());
		}
		...
		{
			throw DNAError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, leftIn[0]->getToken());
		}
		
		if (rightAction->getReturnType()!=Bool)
		{
			throw DNAError("'"+token->getOp()->getText()+"' can only be used with Bools", SOURCE_ERROR, rightIn[0]->getToken());
		}
	...
	{
		throw DNAError("AstOpWithInput::resolveAction called for token '"+token->getOp()->getText()+"', which it shouldn't have been", INTERNAL_ERROR, token);
	}
	else
	{
		throw DNAError("AstOpWithInput made with bad token '"+token->getText()+"'", INTERNAL_ERROR, token);
	}
}
...
void AstToken::resolveAction()
{
	...
	if (token->getType()==TokenData::IDENTIFIER || token->getType()==TokenData::OPERATOR)
	{
		...
		{
			throw DNAError("non overloadable operator in AstToken, it should have been removed and processed by the parser", INTERNAL_ERROR, token);
		}
		...
		if (inLeftType->getType()==TypeBase::TUPLE && inLeftType->getSubType(token->getText()).type!=nullptr)
		{
			...
			{
				throw DNAError("sorry, DNA does not yet support mutating tuples", SOURCE_ERROR, token);
			}
		}
		*/
		...
		catch (IdNotFoundError err)
		{
			...
			{
				throw DNAError("unknown overload for "+inLeftType->getString()+" "+token->getText()+" "+inRightType->getString()+"'", SOURCE_ERROR, token);
			}
			...
			{
				throw DNAError("metatype handeling in "+FUNC+" not yet implemented", INTERNAL_ERROR, token);
			}
			else if (inRightType->isVoid())
			{
				//throw DNAError("unknown identifier '"+token->getText()+"' (var can not be made bc right in type is "+type->getString()+")", SOURCE_ERROR, token);
				throw DNAError("unknown identifier '"+token->getText()+"'", SOURCE_ERROR, token);
			}
			else if (!inRightType->isCreatable())
			{
				throw DNAError("cannot create variable '"+token->getText()+"' of type {"+inRightType->getString()+"}", SOURCE_ERROR, token);
			}
	...
	else if (token->getType()==TokenData::LITERAL || token->getType()==TokenData::STRING_LITERAL)
	{
		if (!inLeftType->isVoid() || !inRightType->isVoid())
		{
			throw DNAError("a literal can not be given an input", SOURCE_ERROR, token);
		}
	...
	{
		throw DNAError("AstToken givin invalid token '"+token->getText()+"' of type "+TokenData::typeToString(token->getType()), INTERNAL_ERROR, token);
	}
}
