...
void CppNameContainer::addPn(const string& pn, const string& cppNameHint)
{
	...
	{
		throw DNAError("Tried to add '"+pn+"' as a pn name to a CppNameContainer but that pn name already exists", INTERNAL_ERROR);
	}
	
	// now we need to find a unique C++ name, the DNA name will almost always be unique, but there may be cases where DNA treats scope differently or uses a keyword or something
...
void CppNameContainer::reserveCpp(const string& cpp, bool ignoreCollisions)
{
	...
	{
		throw DNAError("called CppNameContainer::reserveCpp with id '"+cpp+"', which already exists", INTERNAL_ERROR);
	}
...
string CppNameContainer::getCpp(const string& pn)
{
	...
	if (result==pnToCppMap.end())
	{
		if (parent)
			...
			throw DNAError("could not find C++ equivalent of '"+pn+"' in CppNameContainer::getCppForPn", INTERNAL_ERROR);
	}
	else
...
void CppFuncBase::endln()
{
	if (exprLevel>0)
	{
		throw DNAError("non zero expression level when ending line in C++ program, code so far:\n"+indentString(source), INTERNAL_ERROR);
	}
...
void CppFuncBase::popExpr()
{
	if (exprLevel<=0)
	{
		throw DNAError("CppProgram::popExpression called with zero expressionLevel", INTERNAL_ERROR);
	}
...
void CppFuncBase::pushBlock()
{
	if (exprLevel>0)
	{
		throw DNAError("CppProgram::pushBlock called when expressionLevel was not zero", INTERNAL_ERROR);
	}
...
void CppFuncBase::popBlock()
{
	
	{
		if (blockLevel<=0)
		{
			throw DNAError("CppProgram::popBlock called with zero indentationLevel", INTERNAL_ERROR);
		}
...
{
	globalTopCode+="// this C++ code is transpiled from DNA\n";
	globalTopCode+="// DNA v"+to_string(VERSION_X)+"."+to_string(VERSION_Y)+"."+to_string(VERSION_Z)+" was used\n";
...
string CppProgram::getTypeCode(Type in)
{
	...
	default:
		throw DNAError("CppProgram::getTypeCode called with invalid type "+(TypeBase::getString(in->getType())), INTERNAL_ERROR);
	}
}
...
void CppProgram::addFunc(const string& name, vector<std::pair<string, string>> args, string returnType, string contents)
{
	if (hasFunc(name))
	{
		throw DNAError("called CppProgram::addFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
...
void CppProgram::pushFunc(const string& name, const string& cppNameHint, Type leftIn, Type rightIn, Type returnType)
{
	if (hasFunc(name))
	{
		throw DNAError("called CppProgram::pushFunc with function name '"+name+"', which already exists", INTERNAL_ERROR);
	}
...
void CppProgram::popFunc()
{
	...
	{
		throw DNAError("called CppProgram::popFunc when function wasn't ready", INTERNAL_ERROR);
	}
	...
	if (funcStack.empty())
	{
		throw DNAError("called CppProgram::popFunc too many times", INTERNAL_ERROR);
	}
...
{\n\
	argc = argcIn;\n\
	argv = argvIn;\n\
	if (argc >= 2 && strcmp(argv[1], \"--running-from-DNA\") == 0)\n\
	{\n\
		argc -= 2;\n\
		if (argc == 0)\n\
			argv = 0;\n\
