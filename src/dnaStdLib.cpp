	...
	return [=](Action inLeft, Action inRight, CppProgram* prog)
	{
		...
		do
		{
			...
			else
			{
				...
				{
					throw DNAError("invalid '$' escape in C++ code: "+cppCode, INTERNAL_ERROR);
				}
			}
...
inline T getValFromTuple(void* data, Type type, string name)
{
	...
	if (!a.type)
		throw DNAError("tried to get invalid property '"+name+"' from type "+type->getString(), INTERNAL_ERROR);
...
inline T getValFromTuple(void* data, Type type, int index)
{
	if (index<0 || index>=type->getAllSubTypes()->size())
		throw DNAError("tried to get invalid property #"+to_string(index)+" from tuple "+type->getString(), INTERNAL_ERROR);
...
inline void setValInTuple(void* data, Type type, string name, T val)
{
	...
	if (!a.type)
		throw DNAError("tried to set invalid property '"+name+"' from type "+type->getString(), INTERNAL_ERROR);
...
void populateStringFuncs()
{
	...
	addAction("ascii", Int, Void, String,
		LAMBDA_HEADER
		{
			...
			if (val<0 || val>=256)
			{
				throw DNAError("tried to make ascii string out of value "+to_string(val), RUNTIME_ERROR);
			}
			string out;
			out+=(char)val;
	...
	addAction("at", String, Int, Int,
		LAMBDA_HEADER
		{
			...
			{
				throw DNAError("tried to access location "+to_string(index)+" in string "+to_string(str.size())+" long", RUNTIME_ERROR);
			}
			*out=str[index];
			return out;
		},
	...
	addAction("sub", String, makeTuple(vector<NamedType>{NamedType{"a", Int}, NamedType{"b", Int}}, true), String,
		LAMBDA_HEADER
		{
			...
			{
				throw DNAError("invalid arguments sent to String.sub", RUNTIME_ERROR);
			}
		...
		AstWhatevToActionFactory::make(
			[](Type leftType, Type rightType) -> Action
			{
				...
				return lambdaAction(
					...
					[=](Action inLeft, Action inRight, CppProgram* prog)
					{
						throw DNAError("not yet implemented", INTERNAL_ERROR);
					},
				...
				return lambdaAction(
					...
					[=](void* leftIn, void* rightIn) -> void*
					{
						...
						if (*(int*)rightIn<0 || *(int*)rightIn>=size)
							throw DNAError("index out of bounds, tried to get element at position "+to_string(*(int*)rightIn)+" in array "+to_string(size)+" long", RUNTIME_ERROR);
				...
				return lambdaAction(
					...
					[=](void* leftIn, void* rightIn) -> void*
					{
						...
						if (index<0 || index>=size)
							throw DNAError("index out of bounds, tried to set element at position "+to_string(index)+" in array "+to_string(size)+" long", RUNTIME_ERROR);
...
void populateIntArrayAndFuncs()
{
	...
	addAction("get", IntArray, Int, Int, LAMBDA_HEADER
		{
			...
			if (pos<0 || pos>=size)
				throw DNAError("tried to acces position "+to_string(pos)+" of array "+to_string(size)+" long", RUNTIME_ERROR);
	...
	addAction("set", IntArray, PNCN_Tuple(Int, Int), Void,
		LAMBDA_HEADER
		{
			...
			int size=getValFromTuple<int>(leftIn, IntArray, "_size");
			if (pos<0 || pos>=size)
				throw DNAError("tried to set value at position "+to_string(pos)+" of array "+to_string(size)+" long", RUNTIME_ERROR);
...
void populateCppInterfaceFuncs()
{
	addAction("cppCode", Void, String, Void, LAMBDA_HEADER
		{
			throw DNAError("you can't run interpreter with code that uses 'cppCode'", SOURCE_ERROR);
		},
		ADD_CPP_HEADER
		{
			prog->pushBlock();
	...
	addAction("cppHead", Void, String, Void, LAMBDA_HEADER
		{
			throw DNAError("you can't run interpreter with code that uses 'cppHead'", SOURCE_ERROR);
		},
		ADD_CPP_HEADER
		{
