...
struct Flags
{
	string myPath;					// path to the DNA exeutable that is now running
...
int main(int argc, char ** argv)
{
	...
	if (flags.flagError)
	{
		cout << "try 'DNA -h' for help" << endl;
		return 0;
	}
	
	if (flags.help)
	{
		cout << "DNA v" << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		cout << "usage: DNA [options] [source file] [options]" << endl;
		cout << "options: " << endl;
		cout << "-v, -version      display the version of DNA" << endl;
	...
	if (flags.version)
	{
		cout << "DNA version " << VERSION_X << "." << VERSION_Y << "." << VERSION_Z << endl;
		return 0;
	}
	...
	{
		cout << "multiple source files specified, DNA does not currently support this" << endl;
		cout << "try 'DNA -h' for help" << endl;
		return 0;
	}
		...
		else
		{
			...
			if (!flags.binOutFile.empty() || flags.runCompiled)
			{
				...
				if (flags.runCompiled)
				{
					...
					cmd = "./"+binFileName + " --running-from-DNA " + str::join(cmdLineArgs, " ", false);
