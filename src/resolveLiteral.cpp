...
Action resolveLiteral(Token token)
{
	...
	{
		throw DNAError(FUNC+" called on token that is not a literal", INTERNAL_ERROR, token);
	}
	...
	{
		throw DNAError("tried to make literal with invalid type of " + type->getString(), INTERNAL_ERROR, token);
	}
}
